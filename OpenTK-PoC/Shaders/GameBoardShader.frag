...
const vec4 green = vec4(0.3, 0.55, 0, 1);
const vec4 brown = vec4(0.8, 0.7, 0.5, 1);
const vec4 gray = vec4(0.05, 0.0, 0.0, 1);
...
void main()
{
    vec4 texColor = texture(textureSampler, modelPos.xy);

    vec4 backgroundColor = modelPos.z <= 0 ? mix(brown, green, -modelPos.z * 10) : mix(brown, gray, 2 * modelPos.z);
    vec4 baseColor = (1-0.5) * backgroundColor + 0.5 * texColor;
